...
%******************************************* 
N = 201;                          % Number of Wells, must be odd
...
omgPerp     = 2*pi * 100;          % radial, perpendicular trap freq.
...
Vext_trap = (0.5*mCs*omgLong^2*x.^2)';                % Potential energie  
...
Vext    = Er*((epsilon)*(x/dLat).^2 + delta)';
%Vext   = (0.5*mCs*omgLong^2*x.^2)';                % Potential energie  
K_plus  = (K + Er*((alpha/dLat^2)*x.^2 + (beta/dLat)*x + gamma));
K_minus = (K + Er*((alpha/dLat^2)*x.^2 - (beta/dLat)*x + gamma));
%%
%******************************************* 
...
while (delta > 1e-9)
    ...
    function DPSI2 = DGLFUNC2(t,PSI)
        ...
        %DPNSE  = min(DNL,DNLS);        
        DPSI2=-1/hbar*(Vext+DNLS).*psi_j + ...
               K_plus'.*psi_jp1/hbar     + ...
               K_minus'.*psi_jm1/hbar    ;%+ ...
...
UDD = -4*pi*hbar^2*0.5*a0/mCs/((2*pi)^(3/2))/sigPerp^2/sigLatt;
    ...
    function DPSI= DGLFUNC(t, PSI, tau)
        ...
        DPSI= -1i/hbar*(Vext+DNLS).*psi_j + ...
               besselj(0,3.26)*1i*K_plus'.*psi_jp1/hbar   + ...
               besselj(0,3.26)*1i*K_minus'.*psi_jm1/hbar   ;%+ ...
                                         %-1/hbar*UDD*psi_j.*(n_jm1+n_jp1);
     end
