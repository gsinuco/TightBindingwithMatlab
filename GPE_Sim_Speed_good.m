...
s = 7;                                    % Lattice Depth in Er
a = 5*a0;                                % Scattering length
omgLong = 2*pi * 5;                      % Longitudinal trap frequency
omgPerp = 2*pi * 30;                       % radial, perpendicular trap freq.
...
%******************************************* 
%options=odeset('RelTol',3e-8,'AbsTol',1e-8,'Stats','off','MaxStep',1e-6);
...
while (delta > 1e-6)
    ...
    function DPSI2 = DGLFUNC2(t,PSI)
        ...
        DNL    = n* sqrt( mCs*omgPerp^2*g3D/(sqrt(2*pi)*pi*sigLatt) );
...
F0 =3*mCs*g;
    ...
    function DPSI= DGLFUNC(t, PSI, tau)
        ...
        Vext = (x*Fmod+ 0.5*mCs*omgLong^2*(x).^2)';
        ...
        DPSI=-1i/hbar*(Vext+DNL).*psi_j+1i/hbar*K*(psi_jm1+psi_jp1)+...
            -1i/hbar*UDD*psi_j.*(n_jm1+n_jp1);%+1e-3*n;
    end
...
for counter = 2:nSaved
    ...
    T = 0.0;%modPeriod*(counter-2);                       % start time of modulation period
