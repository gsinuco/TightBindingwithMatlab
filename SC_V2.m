...
for r=2:N_time+1
    ...
    psi(1) = 0.01;    % initial position
    psi(2) = 0.0;    % initial momentum 
    
    
    tSteps = linspace(0,80*(1+r/500),4096);
    

    %
    A           = 1.0*K*hbar/(dLat*mCs*modOmega);
    alpha_tilde = (2.0*pi*omgLong/modOmega)^2;
    A_tilde     = (1+omgLong^2/modOmega^2)*mCs*A*lLat*modOmega/(2*pi*hbar);
    phi         = 0.0;
    J_tilde     = (4.0/sqrt(pi))*(s^0.75)*exp(-2*sqrt(s));
