...
for r=2:N_time+1
    ...
    alpha = 0.5*(2.0*pi*omgLong/modOmega)^2;
    beta  = (1+(omgLong/modOmega)^2)*A*2*pi/dLat;
    phi   = 0.0;
    v_0_tilde = (0.5*s*Er*pi/(dLat*dLat*mCs))*(modPeriod^2);
    
    %propagation of the Schrodinger equation. 
    [TT,PSI_out] = ode45(@(t,y) HamiltonEqs_Full(alpha,beta,v_0_tilde,phi,t,y),tSteps,psi,options);
...
%% 
function dpsi = HamiltonEqs_Full(alpha,beta,v_0_tilde,phi,t,psi)
    dpsi    = zeros(2,1);
    dpsi(1) = psi(2) + beta*sin(2*pi*t+phi);
    dpsi(2) = v_0_tilde*sin(2*pi*psi(1)) - alpha*psi(1);
end
