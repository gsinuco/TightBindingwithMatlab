...
omgLong = 2*pi * 10;                      % Longitudinal trap frequency
...
while (delta > 1e-5)
    ...
    function DPSI2 = DGLFUNC2(t,PSI)
        ...
        DNL    = sqrt(n) * sqrt( mCs*omgPerp^2*g3D/(sqrt(2*pi)*pi*sigLatt) );
        %DNLS   = g3D * n/((2*pi)^(3/2))/sigPerp^2/sigLatt;
        %DPNSE  = min(DNL,DNLS);        
...
vmax = 3e3*dLat;
    ...
    function F = calcForce(t)
        
%         % ramp force
%         if t<RampDuration
%             F = mCs * vmax*modOmega*(t/RampDuration*cos(modOmega*t));
%         else
%             F = mCs * vmax*modOmega * cos(modOmega*t);
%         end
        
%         % integrated velocity ramp
%         if t<RampDuration
%             F = mCs *(t*vmax*modOmega*cos(modOmega*t)/RampDuration + (vmax*sin(modOmega*t))/RampDuration)
%         else
%             F = mCs *modOmega * (vmax*cos(modOmega*t));
%         end

          % direct switch on
          F = mCs *modOmega * (vmax*sin(modOmega*t));
    end
    ...
    function DPSI= DGLFUNC(t, PSI, tau)
        ...
        %Fmod = min(F0,F0/(20*modPeriod)*(tau+t)) * cos( modOmega * (tau + t) );
        %Fmod = min(F0,F0/(20*modPeriod)*(tau+t)) * cos( modOmega * (tau + t) );
        
        
        Vext = (x*calcForce(tau + t) + TrapPropagation * 0.5*mCs*omgLong^2*(x).^2)';
        ...
        %DNLS=g3d*n/((2*pi)^(3/2))/sigma_perp^2/sigma_latt;
        %DPNSE=min(DNL,DNLS);
        ...
        DPSI=-1i/hbar*(Vext+DNL).*psi_j+1i/hbar*K*(psi_jm1+psi_jp1)+...
            -1i/hbar*UDD*psi_j.*(n_jm1+n_jp1) + 1e-9*n;
    end

Steps = 16;
tSteps = linspace(0,modPeriod/2,Steps);
...
for counter = 2:nSaved

    T = modPeriod*(counter-2);                       % start time of modulation period
...
%     width2_q(counter) = 2*sqrt(Erw2x);
    ...
    subplot(3,2,1); 
    % COM
    comVect(counter) = sum(x.*psi_x_2)/sum(psi_x_2);
    plot(tVect(1:counter), comVect(1:counter),'o');
    
    %plot(tVect(1:counter), width_q(1:counter),'o');
    xlabel('Propagation time (ms)');
    %title('Momentum width (kLat) with time.');
    title('Center of mass with time.');
    ...
    axis([-50 50 0 inf]);   
    xlabel('Lattice site #');
	title('Density distribution with lattice sites');
    ...
    xlabel('Propagation time (ms)');
    %plot(qexp,psi_exp_2);
    title('Expansion density with time');
    ...
    xlabel('Propagation time (ms)');
    ylim([-1 1]);
    title('Convoluted quasi-momentum with time');

    sgtitle(sprintf('Time = %.0f ms / %.2fperc',T*1e3, counter/nSaved*100 ) );
