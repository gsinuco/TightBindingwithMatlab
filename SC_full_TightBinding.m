...
gamma_damping =  0.00;
J             = Er*(4.0/sqrt(pi))*(s^0.75)*exp(-2*sqrt(s));
...
for r=2:N_time+1
    ...
    tSteps = linspace(0,80,2048);

    %
    A       = 1.0*K*hbar/(dLat*mCs*modOmega);
    alpha   = (mCs*omgLong*lLat/hbar)*(omgLong/modOmega);
    A_tilde = mCs*modOmega*lLat*A/hbar;
    J_tilde = 2*pi*J/(hbar*modOmega);
    phi   = 0.0;
    
    
    %propagation of the Schrodinger equation. 
    [TT,PSI_out] = ode45(@(t,y) HamiltonEqs_Full(alpha,A_tilde,J_tilde,phi,t,y),tSteps,psi,options);
...
%% 
function dpsi = HamiltonEqs_Full(alpha,A_tilde,J_tilde,phi,t,psi)
    dpsi    = zeros(2,1);
    dpsi(1) =  J_tilde*sin(pi*psi(2));
    dpsi(2) = -A_tilde*cos(2*pi*t+phi) - alpha*psi(1);
end
