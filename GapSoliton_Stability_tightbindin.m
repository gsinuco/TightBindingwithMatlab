function [psi_q_2_vect] = GapSoliton_Stability_tightbinding(K_eff_)
% figure(3)
% subplot(2,1,1)
...
UDD = -4*pi*hbar^2*0.5*a0/mCs/((2*pi)^(3/2))/sigPerp^2/sigLatt;
    ...
    function DPSI= DGLFUNC(t, PSI, tau)
        ...
        DPSI=-1i/hbar*(Vext+DNLS).*psi_j+1.0*1i/hbar*K*besselj(0,K_eff_)*(psi_jm1+psi_jp1)+ ... 
             -1i/hbar*UDD*psi_j.*(n_jm1+n_jp1);
    end
