...
omgLong = 2*pi * 20;                      % Longitudinal trap frequency
...
%*******************************************
%  Add random fluctuations to the phase of 
% of the wave-function
%*******************************************

rng('default');
phase_factor = exp(0.001*1i*2.0*pi*rand(N,1));
...
psi_x = sqrt(psi_x_3).*phase_factor';    
psi_x_2 = psi_x_3;
...
UDD = -4*pi*hbar^2*0.5*a0/mCs/((2*pi)^(3/2))/sigPerp^2/sigLatt;
    ...
    function DPSI= DGLFUNC(t, PSI, tau)
        ...
        if(T<modPeriod*(20-2))
            DPSI=-1i/hbar*(Vext+DNLS).*psi_j+1.0*1i/hbar*K*besselj(0,0)*(psi_jm1+psi_jp1)+ ... 
                 -1i/hbar*UDD*psi_j.*(n_jm1+n_jp1);% - 0.1*n;
        else
            DPSI=-1i/hbar*(Vext+DNLS).*psi_j+1.0*1i/hbar*K*besselj(0,3.26)*(psi_jm1+psi_jp1)+ ... 
             -1i/hbar*UDD*psi_j.*(n_jm1+n_jp1);% - 0.1*n;
        end
    end
...
for counter = 2:nSaved
    ...
    %[bec_x,thermal_x] = AverageDensityMatrixEigen(PSI_out,tSteps)
